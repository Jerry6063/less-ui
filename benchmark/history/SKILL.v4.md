...
- **Rhythm.** Three radii, three spacing steps, two shadows plus none. Write the values down.
...
**Then write the plan as tokens before any other CSS.** The first thing in the stylesheet is a
`:root` block holding the palette hexes, four type sizes (`--t-display`, `--t-heading`, `--t-body`,
`--t-meta`), three radii (`--r-1/2/3`), three spacing steps (`--s-1/2/3`), and two shadows
(`--shadow-rest`, `--shadow-lift`). Every font-size, border-radius, box-shadow, gap, padding, and
margin in the file references one of these. The only literals outside `:root` are `0` and a mobile
media query that redefines a token. Design the tokens right and the tiers hold by construction;
there is no counting pass at the end.
...
ceiling: display, heading, body, meta. In tokens: `--t-display` is at least 3x `--t-meta`,
`--t-heading` at least 1.5x `--t-body`. If two things are both "important", one of them is not.
...
There is no fourth value because there is no fourth token. Fewer tiers reads as more professional.
...
One pass. Read the output once, fix what fails once, ship. Do not loop: when a check fails, change a
token or the element that references it, never add a value. Do not report the list.
...
2. One focal point per screen, set in `--t-display`, at least 3x `--t-meta`.
3. Search the CSS for a literal size after `font-size`, `border-radius`, `box-shadow`, `gap`,
   `padding`, or `margin` outside `:root`, other than `0`. Every hit becomes a token reference.
